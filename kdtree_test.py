...
class Boid:
    ...
    def draw(self, screen):
        ...
        pygame.draw.polygon(
            screen,
            BOID_COLOR,
            [tip.astype(int), left.astype(int), right.astype(int)],
        )

    def update(self, boids, target, avoid_walls, neighbor_indices=None):
        self._apply_target(target)
        self._apply_flocking(boids, neighbor_indices)
        if avoid_walls:
    ...
    def _apply_flocking(self, boids, neighbor_indices=None):
        sep_sum = np.zeros(2)
        align_sum = np.zeros(2)
        coh_sum = np.zeros(2)
        sep_total = align_total = coh_total = 0

        # If KD-tree candidates are provided, only scan those.
        ...
        for item in candidates:
            b = boids[item] if use_indices else item
...
def build_neighbor_lists(boids):
    if not boids:
        return []

    positions = np.array([b.position for b in boids], dtype=float)
    tree = cKDTree(positions)

    max_radius = max(
        max(b.perception, b.align_radius, b.coh_radius)
        for b in boids
    )
...
while True:
    ...
    neighbor_lists = build_neighbor_lists(boids) if USE_KDTREE else [None] * len(boids)

    for i, b in enumerate(boids):
        b.update(boids, mouse_target, False, neighbor_lists[i])
        b.draw(screen)
